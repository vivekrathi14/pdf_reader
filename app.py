...
from htmlTemplates import bot_template, css, expander_css, user_template
...
def handle_userinput(query):
    ...
    for i, message in enumerate(st.session_state.chat_history):
        st.session_state.expander1.write(
...
def main():
    ...
    user_question = st.session_state.col1.text_input(
        "Ask a question on the contents of the uploaded PDF:"
    )
    st.session_state.expander1 = st.session_state.col1.expander(
        "Your Chat", expanded=True
    )
        ...
        with st.spinner("Processing"):
            if st.session_state.pdf_doc is not None:
                with NamedTemporaryFile(suffix="pdf") as temp:
                    ...
                    st.session_state.col1.markdown(
